    ...
    {visible.map((line) => {
      ...
      return <div className={`subtitle-line ${isLeft ? "subtitle-slot-left" : "subtitle-slot-right"}`} key={`${line.id}-${line.slot}`} style={{ opacity: layout.opacity, top: `${(isLeft ? style.line1Y : style.line2Y) * 100}%`, left: isLeft ? `${style.line1X * 100}%` : "auto", right: isLeft ? "auto" : `${style.line2Right * 100}%`, transform: "translateY(-50%)", maxWidth: `${(1 - style.safeAreaLeft - style.safeAreaRight) * 100}%` }}>
        {layout.rows.map((row, rowIndex) => <div className="subtitle-row" key={`${line.id}-${rowIndex}`}>
          {row.units.map((unit) => { const members = unit.groupUnits?.length ? unit.groupUnits : [unit]; return <span className="subtitle-unit" key={unit.id}><span className="subtitle-ruby" style={{ fontSize: `${layout.fontSize * style.rubyScale}px` }}>{unit.ruby || ""}</span><span className="subtitle-ruby subtitle-ruby-2" style={{ fontSize: `${layout.fontSize * style.ruby2Scale}px` }}>{unit.ruby_2 || ""}</span><span className="subtitle-surface" style={{ fontSize: `${layout.fontSize}px` }}>{members.flatMap((member) => { const memberOffset = characterOffsets.get(member.id) || 0; const surfaces = lineLevelTiming ? Array.from(member.surface) : [member.surface]; return surfaces.map((surface, charIndex) => { const progress = lineLevelTiming ? fallbackMemberProgress(line, memberOffset + charIndex, characterCount, renderMs) : member.start_ms !== null && member.end_ms !== null ? unitProgress(member.start_ms, member.end_ms, renderMs) : fallbackMemberProgress(line, charIndex, surfaces.length, renderMs); return <span className="subtitle-char-mask" key={`${member.id}-${charIndex}`}><span className="subtitle-char-layer" style={{ color: style.textColor, textShadow: makeStroke(style.outlineColor, style.outlineWidth) }}>{surface}</span><span className="subtitle-char-layer subtitle-char-active" style={{ color: style.activeColor, textShadow: makeStroke(style.outlineColor, style.outlineWidth), clipPath: `inset(-50% ${100 - progress * 100}% -50% ${progress <= 0 ? "100%" : `-${style.outlineWidth}px`})` }}>{surface}</span></span>; }); })}</span></span>; })}
        </div>)}
